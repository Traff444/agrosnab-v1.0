...
async def test_log_crm_event(monkeypatch, tmp_path):
    ...
    # Log a start event
    event_id = await cart_store.log_crm_event(user_id, 'start', {
        'username': 'testuser',
        'first_name': 'Test',
    })
...
async def test_get_user_events_with_filter(monkeypatch, tmp_path):
    ...
    # Filter by type
    catalog_events = await cart_store.get_user_events(
        user_id, event_types=['catalog_view']
    )
...
@pytest.mark.asyncio
async def test_crm_events_table_created(monkeypatch, tmp_path):
    """Test that crm_events table is created on init."""
    from app import cart_store

    db_path = str(tmp_path / "test_crm_table.sqlite3")
    monkeypatch.setattr(cart_store, "DB_PATH", db_path)
...
@pytest.mark.asyncio
async def test_crm_messages_table_created(monkeypatch, tmp_path):
    """Test that crm_messages table is created on init."""
    from app import cart_store

    db_path = str(tmp_path / "test_crm_messages_table.sqlite3")
    monkeypatch.setattr(cart_store, "DB_PATH", db_path)
